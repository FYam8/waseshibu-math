...
if(migration.includes('CURRENT_DATA_VERSION=7')&&backup.includes('LEVEL2_HISTORY_STORAGE_KEY')&&migration.includes('MIGRATION_BACKUP_STORAGE_KEY')&&migration.includes('更新前バックアップ')&&version.includes("APP_VERSION='0.18.0'")&&publicVersion.includes('"appVersion": "0.18.0"')&&publicVersion.includes('"dataVersion": 7'))ok('v0.18.0・data v7・Level2履歴・migration前バックアップ');else fail('版・データ形式整合')
...
if(read('src/pages/Remediation.tsx').includes('recordLevel2Attempt')&&read('src/pages/Remediation.tsx').includes('selectLevel2Question')&&guided.includes('&q=${encodeURIComponent(q.id)}'))ok('Level2の4問連続正解を元問題の習得状態へ接続');else fail('類題4問の習得接続')
