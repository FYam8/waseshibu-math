...
if(migration.includes('CURRENT_DATA_VERSION=5')&&backup.includes('GUIDED_PROGRESS_STORAGE_KEY')&&migration.includes('MIGRATION_BACKUP_STORAGE_KEY')&&migration.includes('更新前バックアップ')&&version.includes("APP_VERSION='0.16.5'")&&publicVersion.includes('"appVersion": "0.16.5"')&&publicVersion.includes('"dataVersion": 5'))ok('v0.16.5・data v5・migration前バックアップ');else fail('版・データ形式整合')
