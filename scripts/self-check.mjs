...
if(focus.includes('page-fallback')&&focus.includes('問題が切れている・見えない場合はページを広げて確認'))ok('切り抜き失敗時の明示的フォールバック');else fail('問題ページフォールバック')
if(focusRules.includes("'2024-Q2-1'")&&focusRules.includes('hasExactFocusOverride')&&focusRules.includes('focusSlicesFor')&&styles.includes('.page-fallback'))ok('2024大問2の実位置補正と表示位置確認');else fail('実位置補正')
...
if(migration.includes('CURRENT_DATA_VERSION=3')&&backup.includes('GUIDED_REVIEW_STORAGE_KEY')&&version.includes("APP_VERSION='0.12.1'")&&publicVersion.includes('"appVersion": "0.12.1"')&&publicVersion.includes('"dataVersion": 3'))ok('v0.12.1・data v3・既存学習データ保護');else fail('版・データ形式整合')
