...
if(app.includes('"/guided-review"')&&paper.includes('ExamMarkReview')&&guided.includes('FocusedQuestionView')&&reinforcement.includes('sourcePracticeProgress'))ok('採点・解説・固定類題を元問題単位の進捗へ接続');else fail('学習ルート統一')
...
if(reinforcement.includes('固定類題を続ける')&&reinforcement.includes('2019〜2021年度の追加演習')&&reinforcement.includes('残日数には含めません'))ok('補強は元問題ごとの固定類題、旧年度別枠は任意');else fail('固定類題・旧年度任意化')
...
if(read('src/pages/Remediation.tsx').includes('recordLevel2Attempt')&&read('src/pages/Remediation.tsx').includes('selectLevel2Question')&&guided.includes('&q=${encodeURIComponent(q.id)}'))ok('Level2固定セットを元問題の習得状態へ接続');else fail('固定類題の習得接続')

const eta=read('src/targetEta.ts')
if(home.includes('goalDayEstimates')&&home.includes('`あと${estimate.days}日`')&&home.includes('残り {estimate.remainingQuestions}問｜1日10問ペース')&&eta.includes('DEFAULT_DAILY_TASK_CAPACITY=10')&&eta.includes('reservedQuestionCount')&&eta.includes('Math.ceil(remainingQuestions/cap)'))ok('A60/B70/C75の単純残日数UX');else fail('目標別残り日数UX')
