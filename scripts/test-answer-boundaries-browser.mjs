...
try{
  for(const width of [390,1280]){
    ...
    const scoresBefore=await page.evaluate(()=>localStorage.getItem('waseshibu-math-exam-scores'))
    await page.goto(base+'#/guided-review?q=2024-Q1-1&check=1')
    await page.getByPlaceholder('答えを入力',{exact:true}).fill('-13')
    await page.getByRole('button',{name:'確認する',exact:true}).click()
    await page.getByRole('button',{name:'続ける',exact:true}).click()
    await page.reload()
    assert.match(await page.locator('.skill-check .problem').innerText(),/−7−/)
    await page.getByPlaceholder('答えを入力',{exact:true}).fill('5')
    await page.getByRole('button',{name:'確認する',exact:true}).click()
    await page.getByRole('button',{name:'続ける',exact:true}).click()
    assert.match(await page.locator('.skill-check').innerText(),/説明後の別問題は正解/)
    await page.getByRole('button',{name:'元の問題の解説・解き直しへ',exact:true}).click()
    await page.locator('.guided-review-grid').waitFor()
    await page.goto(base+'#/guided-review?q=2022-Q3-2-i&check=1')
    await page.getByRole('button',{name:'3(x+2x)=900',exact:true}).click()
    await page.getByRole('button',{name:'続ける',exact:true}).click()
    await page.getByPlaceholder('答えを入力',{exact:true}).fill('120')
    await page.getByRole('button',{name:'確認する',exact:true}).click()
    await page.getByRole('button',{name:'続ける',exact:true}).click()
    assert.match(await page.locator('.skill-check .problem').innerText(),/リボン/)
    await page.getByPlaceholder('答えを入力',{exact:true}).fill('20')
    await page.getByRole('button',{name:'確認する',exact:true}).click()
    await page.getByRole('button',{name:'続ける',exact:true}).click()
    assert.match(await page.locator('.skill-check').innerText(),/長期定着の評価ではありません/)
    assert.equal(await page.evaluate(()=>localStorage.getItem('waseshibu-math-exam-scores')),scoresBefore)
    assert.deepEqual(errors,[])
    console.log(`PASS ${width}px: real exam input, two incorrect grades, draft reload, persisted answers/score; no cause self-report, unknown cause persisted, practice answer saved, adaptive checks/support/transfer/resume/score isolation; external requests blocked`)
    await context.close()
  }
