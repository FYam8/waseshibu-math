...
try{
  for(const width of [390,1280]){
    ...
    for(const [id,answer] of [['exam-2026-Q1-5','-3^2'],['exam-2026-Q2-3','12,18']]){
      const r=records.find(r=>r.questionId===id);assert.equal(r.status,'wrong');assert.equal(r.answer,answer)
    }
    ...
    console.log(`PASS ${width}px: real exam input, two incorrect grades, draft reload, persisted answers/score; external requests blocked`)
    await context.close()
  }
