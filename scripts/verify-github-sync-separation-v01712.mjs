...
const schoolProfile=read('src/schools/waseshibu/appProfile.ts')
assert.equal(layout.includes('SyncBadge'),false,'SyncBadge must not be rendered')
assert.equal(layout.includes('APP_PROFILE.brand.footer'),true,'Layout must render the school-profile footer')
assert.equal(schoolProfile.includes('学習データはこの端末に保存されます。'),true,'local-only storage message must remain in the WaseShibu profile')
const migration=read('src/dataMigration.ts')
assert.equal(migration.includes('CURRENT_DATA_VERSION=8'),true,'data version must be 8')
...
console.log('GITHUB SYNC SEPARATION VERIFIED: localStorage-only learning flow, data v8 preserved, production bundle clean')
