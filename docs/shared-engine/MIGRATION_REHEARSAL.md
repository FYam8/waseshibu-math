...
Status: Phase-1 read-only rehearsal includes audited activities, daily practice and persisted scheduler plans; production learner-state writes remain legacy
...
6. reject the candidate if any audit reports a mismatch, filtered record, corruption or unmappable value;
7. combine the audited projections into one in-memory `CanonicalLearnerStateMigrationCandidate`;
8. validate canonical exam IDs, target IDs and planner-kind identities against the WaseShibu catalog/profile;
9. reject duplicate exam-result IDs and duplicate activity IDs until explicit no-loss identity policies exist;
10. capture the source strings again and fail if the rehearsal changed any persisted value;
11. return the canonical candidate only as an in-memory value object.

The rehearsal contract marker is version `3` because daily practice and both persisted scheduler plans joined the aggregate candidate. This marker is independent from WaseShibu app/data versions and from the future shared content contract version.
...
The aggregate candidate keeps three concepts separate:

- `dailyPractice` — concrete problem-session progress from `waseshibu-math-daily`;
- `todayRequiredPlan` — persisted Today task membership;
- `studyAheadPlan` — persisted next-day task membership.

The planner audit does **not** invoke high-level reconciliation. The existing 10-task cap, no-11th-refill behavior, target-switch replacement and next-day promotion remain WaseShibu runtime policy and are not executed or rewritten by the migration rehearsal.
...
The test requires byte/string parity for every audited source key after the simulated rollback, including the complete attempts string, daily-practice string and both scheduler-plan strings. It also verifies that still-out-of-scope learner data such as `waseshibu-math-guided-progress-v2` remains untouched.
...
- the active daily reader and canonical daily-practice shadow disagree;
- the read-only legacy planner reader and strict planner shadow disagree;
...
- daily/planner state is malformed or contains unsupported persisted fields;
...
7. `test:shared-engine-dual-read`
8. `test:shared-engine-migration-rehearsal`

A failure in any of these blocks the PR/deploy build.

## Next migration step

Before a production write path is designed, the same shadow/parity/rehearsal pattern must be expanded to prep-check, guided review/progress, remediation and Level2 state, followed by backup/export semantics and IndexedDB/cloud projection. Only after the full backup-relevant state is covered should the branch add a candidate write migration with restore-point creation, atomic rollback and post-write parity verification.
