...
export default function MistakeReview(){
  ...
  return <>
    ...
    {!!examWrong.length&&<section className="card"><div className="section-head"><div><span className="eyebrow">QUESTION BY QUESTION · TARGET ONLY</span><h2>今直す小問</h2></div><b>{examWrong.length}問</b></div><p className="muted">現在の{targetGoalLabel(prefs.target)}目標に含まれる問題だけです。自力で解き直す・ステップで理解する・すぐ答えを見る、のどれからでも始められます。</p><div className="guided-question-list">{examWrong.map(({attempt,meta})=>{const review=loadGuidedProgress(meta.id);return <article key={attempt.questionId}><div><b>{meta.year}年度 大問{meta.major}（{meta.subNo}）</b><span>{meta.topic}</span><small>問題ランク{meta.grade}・{gradeAdvice(prefs.target,meta.grade)}{attempt.mistakeTag?` ／ ${attempt.mistakeTag}`:''}</small>{review&&<em>{guidedOutcomeLabel(review.mastery)}</em>}</div><Link className="button primary" to={`/guided-review?q=${encodeURIComponent(meta.id)}&check=1`}>この1問を理解する</Link></article>})}</div></section>}
