...
export default function MistakeReview(){
  ...
  for(const a of attempts)if(a.questionId.startsWith('mastery-')&&a.status==='correct'&&!masteredAt.has(a.topic))masteredAt.set(a.topic,a.at)
  ...
  const unresolved=[...latest.values()].filter(x=>x.status!=='correct'&&x.at>(masteredAt.get(x.topic)||''))
