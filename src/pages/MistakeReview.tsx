...
export default function MistakeReview(){
  ...
  const examWrong=unresolved.flatMap(attempt=>{const meta=questionMeta(attempt.questionId);return meta&&(!focusYear||meta.year===focusYear)?[{attempt,meta}]:[]}).sort((a,b)=>b.attempt.at.localeCompare(a.attempt.at))
  return <>
    ...
    {!!examWrong.length&&<section className="card"><div className="section-head"><div><span className="eyebrow">QUESTION BY QUESTION</span><h2>間違えた小問を1問ずつ直す</h2></div><b>{examWrong.length}問</b></div><p className="muted">自力で解き直す・ステップで理解する・すぐ答えを見る、のどれからでも始められます。</p><div className="guided-question-list">{examWrong.map(({attempt,meta})=>{const review=loadGuidedReview(meta.id);return <article key={attempt.questionId}><div><b>{meta.year}年度 大問{meta.major}（{meta.subNo}）</b><span>{meta.topic}</span><small>優先度{meta.grade}・{gradeAdvice(prefs.target,meta.grade)}{attempt.mistakeTag?` ／ ${attempt.mistakeTag}`:''}</small>{review&&<em>{guidedOutcomeLabel(review.outcome)}</em>}</div><Link className="button primary" to={`/guided-review?q=${encodeURIComponent(meta.id)}`}>この1問を理解する</Link></article>})}</div></section>}
