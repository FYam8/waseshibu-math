...
export default function GuidedReview(){
  ...
  const persist=(outcome?:GuidedOutcome)=>saveGuidedReview({questionId:q.id,step1,step2,finalAnswer,hintUsed:hint1||hint2,answerSeen,outcome,updatedAt:new Date().toISOString()})
  const checkFinal=()=>{
  ...
  const revealAnswer=()=>{setAnswerSeen(true);setMode('answer');setResult(null);persist(saved?.outcome)}
