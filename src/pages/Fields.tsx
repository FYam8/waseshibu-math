...
export default function Fields(){
  return <>
    ...
    <section className="card notice-box">この分野練習には、まだ解いていない年度に関連する類題も含まれます。関連する類題を見た年度の得点は「参考スコア」として扱います。初見の実力確認には、関連する問題もまだ見ていない評価問題が必要です。</section>
    <section className="field-grid">{level2Fields.map((field,index)=>
      <article className="card field-card" key={field.fieldId}><div className="section-head"><span className="field-number">{String(index+1).padStart(2,'0')}</span><b>{field.masteryEligibleCount}問</b></div><h2>{field.label}</h2><p className="muted">オリジナル：{field.coreQuestionIds.length}問{field.officialPastQuestionIds?.length?<><br/>公式過去問：{field.officialPastQuestionIds.length}問</>:null}{field.supportQuestionIds.length?<><br/>field-support：{field.supportQuestionIds.length}問</>:null}<br/>開始時に2〜4問を固定し、未正解だけを周回します。</p><Link className="button primary" to={`/remediate?field=${encodeURIComponent(field.fieldId)}&topic=${encodeURIComponent(field.label)}`}>この分野を練習</Link></article>
    )}</section>
