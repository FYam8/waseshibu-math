...
import { effectivePracticeStreak, recordPracticeStreak, resetPracticeIfSourceAttemptIsNewer } from '../guidedReview'
...
export default function Remediation(){
  ...
  const latestSourceAttemptAt=sourceQuestion?loadAttempts().filter(a=>a.questionId===`exam-${sourceQuestion}`).sort((a,b)=>b.at.localeCompare(a.at))[0]?.at:undefined
  const [index,setIndex]=useState(0),[answer,setAnswer]=useState(''),[result,setResult]=useState<boolean|null>(null)
  const [streak,setStreak]=useState(()=>sourceQuestion?effectivePracticeStreak(sourceQuestion,latestSourceAttemptAt):0),[total,setTotal]=useState(0),[finished,setFinished]=useState(false)
  ...
  const next=()=>{
    ...
    if(sourceQuestion){
      // 新しい過去問誤答が以前の類題連続正解より後なら、古い3/4等を持ち越さない。
      resetPracticeIfSourceAttemptIsNewer(sourceQuestion,latestSourceAttemptAt)
      recordPracticeStreak(sourceQuestion,result)
    }
    setTotal(v=>v+1)
    if(nextStreak>=4){
