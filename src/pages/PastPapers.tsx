...
export default function PastPapers(){
  ...
  if(phase==='result'&&savedResult){const strategy=savedResult.strategy,priorityWrong=savedResult.wrongItems.filter(item=>gradeInTarget(strategy.target,item.grade)),deferredWrong=savedResult.wrongItems.filter(item=>!gradeInTarget(strategy.target,item.grade));return <>
    ...
    <section className={`card target-result ${strategy.reached?'reached':''}`}><div className="section-head"><div><span className="eyebrow">TARGET {strategy.target}</span><h2>{strategy.reached?`${strategy.target}点目標に到達`:`目標まであと${strategy.gap}点`}</h2></div><b className="target-projection">{strategy.candidates.length?`優先${strategy.candidates.length}問をすべて正解した場合 約${strategy.projectedScore}点`:'小問別データなし'}</b></div><p>{strategy.summary}</p>{strategy.candidates.length>0&&<><h3>次に確認する3問</h3><div className="recovery-list">{strategy.candidates.map((item,i)=><article key={item.key}><strong>{i+1}</strong><div><b>{item.label}</b><small>優先度{item.grade}・約{item.points}点　{item.reason}</small></div></article>)}</div></>}<div className="time-plan"><b>目標別の時間配分</b><div>{strategy.timePlan.map(item=><span key={item.label} style={{flex:item.percent}}>{item.label}<small>{item.percent}%</small></span>)}</div></div><p className="muted">時間配分は学習上の目安です。表示点は優先問題の配点合計から出した条件付き上限で、すでに回収可能と確認された点ではありません。正誤判定と実得点は目標設定によって変わりません。1〜2週間後にも未見の別年度または異設定問題を補助なしで再測定します。</p></section>
