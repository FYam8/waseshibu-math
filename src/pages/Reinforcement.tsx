...
export default function Reinforcement(){
  ...
  return <>
    ...
    <section className={`card route-complete ${done?'done':''}`}><h2>{done?'補強が完了しました':source===2026?'仕上げ補強を完了します':'次年度は補強完了後に解放'}</h2><p>{done?(source===2024?'2025年度で改善を確認します。':source===2025?'2026年度で仕上がりを確認します。':'2026年度の残った弱点も補強できました。必要なら任意演習へ進みます。'):'元の誤答確認、各分野の該当過去問、類題4問連続正解を順に完了します。'}</p>{done?(source===2026?<Link className="button primary" to="/">ホームで仕上がりを確認</Link>:<Link className="button primary" to={`/past-papers?year=${source===2024?2025:2026}`}>{source===2024?'2025年度 改善確認へ':'2026年度 仕上げへ'}</Link>):<Link className="button" to="/">ホームで進捗を見る</Link>}</section>
  </>
}
