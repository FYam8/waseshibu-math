...
import { gradeInTarget, storedExamItems, targetGoalLabel, targetProfile, weakFieldsForStoredExam } from '../targetStrategy'
...
export default function Reinforcement(){
  ...
  const sourceItems=storedExamItems(exam,attempts).filter(item=>item.status!=='correct'&&gradeInTarget(target,item.grade))
  ...
  return <>
    ...
    <div className="reinforce-fields">{fields.map((fieldName,fieldIndex)=>{
      ...
      const sourceItem=sourceItems.filter(item=>classifyRemediationField(item.topic).title===fieldName).sort((a,b)=>({A:1,B:2,C:3}[b.grade]-{A:1,B:2,C:3}[a.grade])||a.key.localeCompare(b.key))[0]
      const remediationLink=`/remediate?topic=${encodeURIComponent(fieldName)}&source=${source}${sourceItem?`&q=${encodeURIComponent(sourceItem.key)}`:''}`
