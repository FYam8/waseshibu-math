...
import { ensureReinforcementPlan, hasCurrentPracticeMastery, latestExam, loadLearningRoute, markOldQuestionCompleted, nextLearningAction, oldQuestionBank, reinforcementComplete, sourceMistakeProgress, yearRole } from '../learningRoute'
...
import { resolveLevel2FieldId } from '../data/level2Data'
...
export default function Reinforcement(){
  ...
  if(!done&&sourceReviewRequired&&!sourceProgress.complete)return <>
    ...
    <section className="card source-review-gate"><h2>残り {sourceProgress.remainingIds.length}問</h2><p>{targetGoalLabel(target)}目標では、元の未解決問題を先に直してから「2019〜2021該当問題 → 負荷別の固定類題セット」へ進みます。</p><div className="reinforce-flow four-step"><b>1　元の未解決問題を再現</b><span>→</span><b>2　旧年度の該当問題</b><span>→</span><b>3　類題2〜4問</b><span>→</span><b>4　次年度で確認</b></div><div className="actions"><Link className="button primary" to={`/mistakes?year=${source}`}>元の未解決問題を直す</Link><Link className="button" to="/">ホームへ戻る</Link></div><p className="muted">答えを見ただけでは完了になりません。「ヒントなしで再現」または「答え確認後に再現」まで進めると、この補強が解放されます。</p></section>
  </>

  return <>
    <div className="page-head"><div><span className="eyebrow">TARGETED REINFORCEMENT · {targetGoalLabel(target)}</span><h1>{source}年度から見つかった弱点3分野</h1><p className="muted">{targetProfile(target).summary} 元の未解決問題 → 過去問の該当問題 → 負荷別の固定類題セットの順で、設定が変わっても解ける状態にします。</p></div><b className="route-status">{done?'補強完了':'補強中'}</b></div>
    ...
    <div className="reinforce-flow four-step"><b>1　元の未解決問題を再現</b><span>→</span><b>2　旧年度の該当問題</b><span>→</span><b>3　類題2〜4問を完了</b><span>→</span><b>4　次年度で確認</b></div>
    ...
    <div className="reinforce-fields">{fields.map((fieldName,fieldIndex)=>{
      ...
      const requiredCount=requiredPracticeCount(triggerSourceQuestionId||null,resolveLevel2FieldId(fieldName)||'')
      const mastered=hasCurrentPracticeMastery(fieldName,exam.at)
      return <section className="card reinforce-field" key={fieldName}><div className="section-head"><div><span className="field-number">{fieldIndex+1}</span><h2>{fieldName}</h2></div><b>{actualDone?'過去問済み':'過去問演習中'} / {mastered?'類題済み':'類題未完了'}</b></div><p className="muted">この診断で予約した問題だけを表示します。同じ問題を別の補強で重複使用しません。</p>
        <div className="target-question-list">{items.length===0?<p className="notice-box">この分野の未使用問題は残っていません。固定類題セットへ進んでください。</p>:items.map(item=>{const isOpen=open===item.id,isDone=completed.has(item.id),graded=item.id in results,meta=guidedQuestion(item.id),expected=getExamAnswer(item.id),scopeNote=examScopeNote(item.year);return <article className={`target-question ${isDone?'done':''}`} key={item.id}><button className="target-question-head" onClick={()=>setOpen(isOpen?null:item.id)}><span>{isDone?'✓':'○'}</span><b>{item.year}年度{yearRole(item.year)==='different-structure'?'（構成が異なる年度）':''} 大問{item.major}（{item.subNo}）</b><small>{source}年度の「{fieldName}」補強・{item.topic}</small><em>{isOpen?'閉じる':isDone?'記録済み・復習':'1問だけ解く'}</em></button>{isOpen&&<div className="target-workspace one-question-reinforce"><div>{scopeNote&&<div className="notice-box exam-scope-notice"><b>この年度の試験範囲</b><p>{scopeNote}</p></div>}{meta?<FocusedQuestionView year={meta.year} major={meta.major} subIndex={meta.subIndex} subCount={meta.subCount} subNo={meta.subNo} topic={meta.topic}/>:<div className="notice-box">この小問を特定できません。間違い直し画面から開き直してください。</div>}</div><aside><div className="reinforce-focus-note"><b>表示・正答はこの1問だけ</b><span>別小問や公式解答ページ全体は表示しません。</span></div><label>（{item.subNo}）の答え</label><MathAnswerInput value={answers[item.id]||''} onChange={value=>setAnswers(v=>({...v,[item.id]:cleanAnswerInput(value)}))} onEnter={()=>{if(!isDone&&!showAnswer[item.id]&&(answers[item.id]||'').trim())autoMark(item.id,item.topic)}} placeholder="答えを入力（全角可）" disabled={isDone}/>{graded&&<div className={`auto-grade ${results[item.id]?'correct':'wrong'}`}><b>{results[item.id]?'○ 正解':'× 不正解'}</b><span>この小問の正答：{expected?.answer||'正答データなし'}</span></div>}<button className="button primary" disabled={isDone||showAnswer[item.id]||!(answers[item.id]||'').trim()} onClick={()=>autoMark(item.id,item.topic)}>{isDone?'採点済み':showAnswer[item.id]?'正答を隠してから再現':'この1問を自動採点'}</button><button className="button" onClick={()=>{
        ...
        <div className="reinforce-next"><div><b>過去問 {ids.filter(id=>completed.has(id)).length}/{ids.length}</b><span> → </span><b>類題 {mastered?`${requiredCount}/${requiredCount}`:`0〜${requiredCount-1}/${requiredCount}`}</b></div><Link className={`button ${actualDone?'primary':'disabled'}`} aria-disabled={!actualDone} onClick={e=>{if(!actualDone)e.preventDefault()}} to={`/remediate?topic=${encodeURIComponent(fieldName)}&source=${source}${triggerSourceQuestionId?`&q=${encodeURIComponent(triggerSourceQuestionId)}`:''}`}>{mastered?'固定セットでもう一度':`類題${requiredCount}問へ進む`}</Link></div>
      </section>})}</div>
    <section className={`card route-complete ${done?'done':''}`}><h2>{done?'補強が完了しました':source===2026?'仕上げ補強を完了します':'次の確認は補強完了後に解放'}</h2><p>{done?`次の推奨は「${nextAfterDone?.label||'ホームで進捗を確認'}」です。未露出の確認年度が残っている場合は、そちらを優先します。`:'元の未解決問題確認、各分野の該当過去問、固定した類題セットの全問正解を順に完了します。'}</p>{done&&nextAfterDone?<Link className="button primary" to={nextAfterDone.to}>{nextAfterDone.label}</Link>:<Link className="button" to="/">ホームで進捗を見る</Link>}</section>
  </>
}
