  ...
  return <>
    ...
    <div className="reinforce-fields">{fields.map((fieldName,fieldIndex)=>{
      ...
      return <section className="card reinforce-field" key={fieldName}><div className="section-head"><div><span className="field-number">{fieldIndex+1}</span><h2>{fieldName}</h2></div><b>{actualDone?'過去問済み':'過去問演習中'} / {mastered?'類題済み':'類題未完了'}</b></div><p className="muted">この診断で予約した問題だけを表示します。同じ問題を別の補強で重複使用しません。</p>
        <div className="target-question-list">{items.length===0?<p className="notice-box">この分野の未使用問題は残っていません。類題4問へ進んでください。</p>:items.map(item=>{const isOpen=open===item.id,isDone=completed.has(item.id);return <article className={`target-question ${isDone?'done':''}`} key={item.id}><button className="target-question-head" onClick={()=>setOpen(isOpen?null:item.id)}><span>{isDone?'✓':'○'}</span><b>{item.year}年度 大問{item.major}（{item.subNo}）</b><small>{item.topic}</small><em>{isOpen?'閉じる':isDone?'記録済み・復習':'同じ画面で解く'}</em></button>{isOpen&&<div className="target-workspace"><div><div className="exam-images">{(examPages[item.year]?.[item.major-1]||[]).map(page=><img key={page} src={paperImage(item.year,page)} alt={`${item.year}年度 大問${item.major}`} />)}</div></div><aside><label>（{item.subNo}）の答え<input value={answers[item.id]||''} onChange={e=>setAnswers(v=>({...v,[item.id]:e.target.value}))} placeholder="答えを入力" /></label><button className="button" onClick={()=>setShowAnswer(v=>({...v,[item.id]:!v[item.id]}))}>{showAnswer[item.id]?'公式解答を閉じる':'公式解答を確認'}</button>{showAnswer[item.id]&&<div className="official-answer">{Array.from({length:answerPages[item.year]||1},(_,i)=><img key={i} src={answerImage(item.year,i+1)} alt={`${item.year}年度 公式解答`} />)}</div>}<p>{isDone?'この問題は記録済みです。必要なら解き直して確認できます。':'公式解答と照合して記録してください。'}</p><div className="actions"><button className="button primary" disabled={isDone} onClick={()=>mark(item.id,item.topic,true)}>{isDone?'記録済み':'正解として記録'}</button><button className="button" disabled={isDone} onClick={()=>mark(item.id,item.topic,false)}>不正解として記録</button></div></aside></div>}</article>})}</div>
