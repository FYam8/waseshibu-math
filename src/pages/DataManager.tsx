...
export default function DataManager(){
  ...
  return <>
    ...
    <section className="card data-card"><div><span className="eyebrow">EXPORT</span><h2>学習データを書き出す</h2><p>端末の故障・機種変更に備えて、解答、自動採点、タイマー、迷い印、得点、弱点、補強進捗、途中状態を1つのJSONファイルに保存します。</p><div className="data-stats"><span>解答記録 <b>{nowStats.attempts}</b></span><span>年度診断 <b>{nowStats.scores}</b></span><span>途中年度 <b>{nowStats.drafts}</b></span></div></div><button className="button primary" onClick={()=>download(collectBackup())}>この端末のデータを書き出す</button></section>
