...
export default function MathAnswerInput({value,onChange,onEnter,disabled,autoFocus,placeholder='答えを入力'}:Props){
  ...
  return <div className="math-answer">
    <input ref={ref} className="answer-input" value={value} onChange={e=>onChange(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')onEnter?.()}} placeholder={placeholder} disabled={disabled} autoCapitalize="off" autoCorrect="off" spellCheck={false}/>
    ...
    <p className="math-help">入力例：分数 <b>3/5</b>　根号 <b>3√2</b>　座標 <b>(4,3)</b>　比 <b>2:3</b></p>
  </div>
}
