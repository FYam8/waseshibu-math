...
export default function ExamMarkReview({year,majors,answers,flags,causeMap,statusFor,setCauseMap,setOverrides,onFinish}:Props){
  ...
  const safeIndex=Math.max(0,Math.min(wrongFirst.length-1,currentIndex)),current=wrongFirst[safeIndex]
  ...
  const move=(next:number)=>{setCurrentIndex(Math.max(0,Math.min(wrongFirst.length-1,next)));window.scrollTo({top:0,behavior:'smooth'})}
  return <>
    <div className="page-head"><div><span className="eyebrow">ONE QUESTION MARKING</span><h1>{year}年度｜採点確認</h1><p className="muted">失点した小問から1問ずつ確認します。ほかの問題・正答は同時に表示しません。</p></div><b>{safeIndex+1} / {wrongFirst.length}</b></div>
    ...
    <div className="mark-focus-grid">
      ...
      <aside className="card mark-focus-panel">
        ...
        <div className="mark-focus-nav"><button className="button" disabled={safeIndex===0} onClick={()=>move(safeIndex-1)}>← 前の小問</button>{safeIndex<wrongFirst.length-1?<button className="button primary" onClick={()=>move(safeIndex+1)}>次の小問 →</button>:<button className="button primary" onClick={onFinish}>採点結果を保存</button>}</div>
