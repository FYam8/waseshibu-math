  ...
  return <>
    ...
    <div className="mark-focus-grid">
      ...
      <aside className="card mark-focus-panel">
        ...
        {status!=='correct'&&<><label className="mark-cause">最初に起きた失点原因（任意・自己申告）<select value={causeMap[current.key]||''} onChange={e=>setCauseMap(v=>({...v,[current.key]:e.target.value}))}><option value="">選択しない</option>{causes.map(x=><option key={x}>{x}</option>)}</select></label><p className="muted">単元名ではなく、最初に崩れた行動を選びます。例：相似には気づいたが対応辺を取り違えた、二次方程式は解けたが片方の解を書き忘れた。これは自己申告であり、自動診断ではありません。</p><Link className="button primary" to={`/guided-review?q=${encodeURIComponent(current.key)}`}>この1問の解説・解き直しへ</Link></>}
