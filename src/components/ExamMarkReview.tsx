...
export default function ExamMarkReview({year,majors,answers,flags,causeMap,overrides,statusFor,setCauseMap,setOverrides,onFinish}:Props){
  const all=majors.flatMap(major=>major.subquestions.map((sub,subIndex)=>({major,sub,subIndex,key:keyFor(major,sub.no)})))
  const wrongFirst=[...all].sort((a,b)=>{
  ...
  const [first,...rest]=wrongFirst
  const currentKey=typeof window!=='undefined'?new URLSearchParams(window.location.hash.split('?')[1]||'').get('markq'):null
  const current=wrongFirst.find(x=>x.key===currentKey)||first
  ...
  const currentIndex=wrongFirst.findIndex(x=>x.key===current.key),status=statusFor(current.key),expected=getExamAnswer(current.key)
  const move=(next:number)=>{
    const target=wrongFirst[Math.max(0,Math.min(wrongFirst.length-1,next))]
    if(!target)return
    const hash=window.location.hash.split('?')[0],params=new URLSearchParams(window.location.hash.split('?')[1]||'')
    params.set('markq',target.key)
    history.replaceState(null,'',`${location.pathname}${location.search}${hash}?${params}`)
    window.dispatchEvent(new HashChangeEvent('hashchange'))
  }
  return <>
    <div className="page-head"><div><span className="eyebrow">ONE QUESTION MARKING</span><h1>{year}年度｜採点確認</h1><p className="muted">失点した小問から1問ずつ確認します。ほかの問題・正答は同時に表示しません。</p></div><b>{currentIndex+1} / {wrongFirst.length}</b></div>
    ...
    <div className="mark-focus-grid">
      ...
      <aside className="card mark-focus-panel">
        ...
        <div className="mark-focus-nav"><button className="button" disabled={currentIndex===0} onClick={()=>move(currentIndex-1)}>← 前の小問</button>{currentIndex<wrongFirst.length-1?<button className="button primary" onClick={()=>move(currentIndex+1)}>次の小問 →</button>:<button className="button primary" onClick={onFinish}>採点結果を保存</button>}</div>
