...
import { APP_EVENT_NAMES, APP_PROFILE } from '../appConfig'
...
export default function Layout({ children }: PropsWithChildren) {
  ...
  useEffect(()=>{
    ...
    try{channel=new BroadcastChannel(APP_PROFILE.runtime.updateChannel);channel.onmessage=event=>{if(event.data?.version&&event.data.version!==APP_VERSION)blocked()}}catch{/* optional */}
    window.addEventListener(APP_EVENT_NAMES.routeChange,refresh);window.addEventListener(APP_EVENT_NAMES.writeBlocked,blocked);window.addEventListener('storage',storageChanged)
    return()=>{window.removeEventListener(APP_EVENT_NAMES.routeChange,refresh);window.removeEventListener(APP_EVENT_NAMES.writeBlocked,blocked);window.removeEventListener('storage',storageChanged);channel?.close()}
  },[])
  ...
  return (
    <div className="app-shell">
      ...
      <header className="topbar">
        <div>
          <div className="brand">{APP_PROFILE.brand.title} <span className="unofficial">{APP_PROFILE.brand.unofficialLabel}</span></div>
          <div className="subtitle">{APP_PROFILE.brand.subtitle}</div>
        </div>
