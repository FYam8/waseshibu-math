...
export type BackupPackage={app:'waseshibu-math';schemaVersion:1|2|3|4|5;dataVersion:number;exportedAt:string;data:Partial<Record<BackupKey,unknown>>}
...
export function collectBackup(storage:StorageLike=localStorage):BackupPackage{
  ...
  return {app:'waseshibu-math',schemaVersion:5,dataVersion:CURRENT_DATA_VERSION,exportedAt:new Date().toISOString(),data}
}

export function validateBackup(value:unknown):BackupPackage{
  if(!isObject(value)||value.app!=='waseshibu-math'||typeof value.schemaVersion!=='number'||value.schemaVersion<1||value.schemaVersion>5||typeof value.exportedAt!=='string'||Number.isNaN(Date.parse(value.exportedAt))||!isObject(value.data))throw new Error('このアプリのバックアップ形式ではありません')
  ...
  return {app:'waseshibu-math',schemaVersion:5,dataVersion:CURRENT_DATA_VERSION,exportedAt:value.exportedAt,data:filtered}
}
...
export function mergeBackupValue(key:BackupKey,local:unknown,incoming:unknown){
  ...
  if(key===LEVEL2_HISTORY_STORAGE_KEY){
    ...
    const attempts=uniqueById((a.attempts||[]).map((x:any)=>({...x,id:x.attemptId})),(b.attempts||[]).map((x:any)=>({...x,id:x.attemptId}))).map(({id,...x}:any)=>x)
    const questionStats:Record<string,any>={}
    for(const x of attempts){const old=questionStats[x.questionId]||{attemptCount:0,correctCount:0,qualifyingCorrectCount:0,lastAttemptAt:null,lastResult:null},qualifying=x.isFirstSubmissionForPresentation&&x.isCorrect&&!x.usedHintBeforeAnswer&&!x.usedExplanationBeforeAnswer&&!x.revealedAnswerBeforeAnswer;questionStats[x.questionId]={attemptCount:old.attemptCount+1,correctCount:old.correctCount+(x.isCorrect?1:0),qualifyingCorrectCount:old.qualifyingCorrectCount+(qualifying?1:0),lastAttemptAt:!old.lastAttemptAt||old.lastAttemptAt<x.answeredAt?x.answeredAt:old.lastAttemptAt,lastResult:!old.lastAttemptAt||old.lastAttemptAt<=x.answeredAt?x.isCorrect:old.lastResult}}
    const sessions:Record<string,any>={...(a.sessions||{})}
    for(const [sessionKey,value] of Object.entries(b.sessions||{}) as [string,any][])if(!sessions[sessionKey]||String(sessions[sessionKey].updatedAt||'')<String(value.updatedAt||''))sessions[sessionKey]=value
    const events=[...(Array.isArray(a.masteryEvents)?a.masteryEvents:[]),...(Array.isArray(b.masteryEvents)?b.masteryEvents:[])]
    const masteryEvents=[...new Map(events.map((x:any)=>[`${x.fieldId}|${x.achievedAt}|${(x.questionIds||[]).join(',')}`,x])).values()]
    return {schemaVersion:1,attempts,questionStats,sessions,masteryEvents}
  }
  if(key.endsWith('exam-drafts-v2')||key.endsWith('daily-required-plan-v2')||key.endsWith('study-ahead-plan-v1')||key===GUIDED_REVIEW_STORAGE_KEY||key===GUIDED_PROGRESS_STORAGE_KEY||key===REMEDIATION_PROGRESS_STORAGE_KEY)return {...(isObject(local)?local:{}),...(isObject(incoming)?incoming:{})}
