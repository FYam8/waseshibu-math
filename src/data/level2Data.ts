...
const explanationOverrides:Record<string,string>={
  ...
  'L2-2026-Q3-2':'AとBの2枚だけを順に考えると、全体は9×8=72通り。BがAの倍数になる組をAごとに数える。A=1ならB=2,3,4,5,6,7,8,9の8通り、A=2ならB=4,6,8の3通り、A=3ならB=6,9の2通り、A=4ならB=8の1通り。A=5,6,7,8,9では1〜9の範囲にAと異なる倍数はない。したがって条件を満たすのは8+3+2+1=14通り。よって確率は14/72=7/36。',
  'L2-2026-Q3-3':'A,B,Cの順序つき全体は9×8×7=504通り。CがAの倍数かつBの倍数になるには、AとBがともにCの約数で、しかもA,B,Cは異なる必要がある。Cを固定して数える。C=4では使える約数は1,2なので(A,B)は2通り。C=6では1,2,3から異なる2つを順に選ぶので3×2=6通り。C=8では1,2,4から6通り。C=9では1,3から2通り。C=1,2,3,5,7では使えるC未満の約数が2個未満なので0通り。合計2+6+6+2=16通り。したがって確率は16/504=2/63。',
  'L2-2026-Q5-3':'前問よりAC=12、AD=3、DC=9、BC=12。またAB∥DEなので△CDE∽△CBAで、DE:AB=CE:CB=9:12=3:4、よってDE=(3/4)AB。四角形ABEDは円に内接する台形なので等脚台形で、AD=BE=3、対角線AE=BD。条件AB=BDより、AB=BD=AE=xとおく。円に内接する四角形の関係「AB×DE+AD×BE=AE×BD」を使うと、x×(3x/4)+3×3=x×x。したがって(3/4)x²+9=x²、x²=36よりAB=BD=6。次にD,A,Cは一直線、B,D,Fは一直線、AB∥CFなので△DAB∽△DCF。AD:DC=AB:CF=3:9より3:9=6:CF。したがってCF=18。'
}
